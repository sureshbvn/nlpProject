...
def wordVectorModel():
     string = open("input_3.txt").read()
     cleaned_str = re.sub('[^a-zA-Z0-9\n\.\,\x7f-\xff]', ' ', string)
     cleaned_str = cleaned_str.replace('  ',' ').replace(' ,',',')
     #vecModel = wordVectorModel(cleaned_str)

     trainingText = util.cleanWordVec(cleaned_str)
     exclude = set(mstring.punctuation)
     trainingText = ''.join(ch for ch in trainingText if ch not in exclude)
     trainingTextInput = util.getListWordVec(trainingText)
     model = gensim.models.Word2Vec(trainingTextInput,size = 5,min_count = 1,max_vocab_size=None)
     print(len(model.vocab))
     model.save("save_word_model.txt")
     return model
